...
<xsl:template name="note">
	<xsl:choose>
		...
		<xsl:when test="@id = 'global-ajax-event'">
			As of jQuery 1.9, all the handlers for the <a href="/category/ajax/global-ajax-event-handlers/">jQuery global Ajax events</a>, including those added with the <code><xsl:value-of select="@data-title"/></code> method, must be attached to <code>document</code>.
		</xsl:when>
	</xsl:choose>
</xsl:template>
</xsl:stylesheet>
